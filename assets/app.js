...
;(function () {
  ...
  // loadFontLibrary(): fetches data/fonts.json (pre-rendered by a one-off
  // backend script -- one small SVG type-specimen per font, straight from
  // the app's own renderer, so what a visitor sees here IS what plots) and
  // renders a filterable card grid. Category filter reuses the same
  // .pill/.pill.active look as the quote form's radio pills, wired by hand
  // here since these buttons are static markup, not generated by
  // fillPillGroup. Empty/missing data renders nothing (no gallery to show
  // beats a broken one) rather than an error.
  let FONT_LIST = []
  function renderFontGallery(category) {
    const gallery = $('font-gallery')
    if (!gallery) return
    gallery.innerHTML = ''
    const shown = category === 'all' ? FONT_LIST : FONT_LIST.filter((f) => f.category === category)
    shown.forEach((f) => {
      const card = document.createElement('div')
      card.className = 'font-card'
      const img = document.createElement('img')
      img.src = f.file
      img.alt = f.name + ' sample'
      img.loading = 'lazy'
      ...
      cat.className = 'font-card-cat'
      cat.textContent = f.category === 'ems' ? 'EMS' : f.category === 'hershey' ? 'Hershey' : 'Commercial'
  ...
  async function loadFontLibrary() {
    const filterEl = $('font-filter')
    if (!$('font-gallery')) return
    try {
      const res = await fetch('data/fonts.json', { cache: 'no-store' })
      if (res.ok) {
        const parsed = await res.json()
        if (Array.isArray(parsed)) FONT_LIST = parsed
      }
    } catch (e) { /* offline / no file yet -- gallery just stays empty */ }
    renderFontGallery('all')
    if (filterEl) {
      filterEl.addEventListener('click', (e) => {
        ...
        btn.classList.add('active')
        renderFontGallery(btn.dataset.cat)
      })
    }
  }
  ...
  // Gates the Messenger link on TWO independent conditions: the site owner
  // has configured MESSENGER_PAGE_USERNAME, AND the visitor has ticked the
  // confidentiality agreement (#f-agree-confidential). Either missing keeps
  // book-btn looking/behaving disabled (same aria-disabled pattern as
  // before) -- one function owns both href and aria-disabled so they can
  // never fall out of sync with each other.
  function agreedToConfidentiality() {
    const box = $('f-agree-confidential')
    return box ? box.checked : true // no checkbox in the DOM -- don't block on something that doesn't exist
  }
  function updateBookAvailability() {
    const href = messengerHref()
    const agreed = agreedToConfidentiality()
    document.querySelectorAll('[data-messenger-link]').forEach((el) => {
      if (href && agreed) {
        ...
        el.title = !href ? 'Messenger link not configured yet' : 'Please agree to the confidentiality notice above first'
      }
    })
    if (!href) {
  ...
  // Lightbox: tap a proof/sample photo to see it full-screen, tap anywhere
  // (backdrop or the photo itself) to close. One delegated listener on
  // <main> instead of per-image handlers, since both galleries load their
  // <img> tags asynchronously (loadProofPhotos/loadHandwritingSamples) --
  // delegation means new photos work automatically with no re-wiring.
  ...
  async function init() {
    ...
    $('book-btn').addEventListener('click', (e) => {
      if (!messengerHref() || !agreedToConfidentiality()) { e.preventDefault(); return }
      copySummary()
    })
    if ($('f-agree-confidential')) {
      $('f-agree-confidential').addEventListener('change', updateBookAvailability)
    }
